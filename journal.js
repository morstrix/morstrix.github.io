document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadTopPlayers() {
        ...
        try {
            ...
            snapshot.forEach(doc => {
                ...
                html += `
                    <div style="display: flex; justify-content: space-between; gap: 15px; margin-bottom: 8px;">
                        <span style="color: #a84d6b; width: 25px;">${rank}.</span>
                        <span style="color: #fff; flex: 1; text-align: left;">${name.slice(0, 10)}</span>
                        <span style="color: #ffb7c7; width: 50px; text-align: right;">${score}</span>
                    </div>
                `;
                rank++;
            });
            ...
            container.innerHTML = '<div style="color:#a84d6b; text-align:center;">⚠️ UNAVAILABLE ⚠️</div>';
        }
    }
